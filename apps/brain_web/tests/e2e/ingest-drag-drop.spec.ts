 ...
 *
 * Plan 16 Task 1 (D1): the production race between ``InboxScreen``'s
 * mount-time ``loadRecent()`` fetch and the optimistic row inserted by
 * ``setInputFiles`` is now fixed at the source — ``inbox-store.loadRecent``
 * does an id-keyed merge that preserves in-flight optimistic ``queued``
 * rows instead of overwriting them. The Plan 14 Task 6 ``waitForResponse``
 * band-aid (armed before ``page.goto``) is no longer needed and has
 * been removed; ``waitForLoadState("networkidle")`` is sufficient.
 *
 * Flow:
 *   1. Navigate to ``/inbox``.

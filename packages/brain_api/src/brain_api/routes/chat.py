...
Plan 05 Task 17 landed the bare handshake; Task 18 replaces the dict-
shaped sends and the ``ack`` echo with typed Pydantic events + parsed
client messages. Task 19 will wire ``ChatSession`` into the loop and
emit real turn events; Tasks 20-21 add cancellation and reconnect.

The endpoint does four things today:
...
3. After accept, emits two typed framing events —
   ...
   receive loop that parses every inbound frame into a
   ``ClientMessage`` variant via ``parse_client_message``. Unknown or
   malformed messages get a typed ``ErrorEvent`` back; the socket
   stays open (``recoverable=True``).
4. Task 19 will dispatch parsed ``ClientMessage`` variants into a
   ``SessionRunner``; Task 18 just parses + logs.
...
async def chat_ws(websocket: WebSocket, thread_id: str) -> None:
    ...
    Task 18: handshake + typed-event receive loop. Task 19 fills in
    real turn-running via ``SessionRunner``.
    """
    ...
    # Task 18 emits defaults so the handshake shape is locked.
    ...
    try:
        while True:
            ...
            try:
                ...
                # intentional: Pydantic raises ``ValidationError`` but
                # any other deserialization slip (e.g., ``raw`` is not
                # a dict) should map to the same wire-level error so
                # the client gets a uniform response regardless of
                # which validator tripped.
                await websocket.send_json(
                    serialize_server_event(
                        ErrorEvent(

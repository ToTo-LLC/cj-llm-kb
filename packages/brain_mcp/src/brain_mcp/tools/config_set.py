...
than the rest of Task 19 combined. Plan 04 ships the MCP tool surface so
clients (Claude Desktop, browser) can discover it; actual persistence lands in
...
Safety layers (both applied before any state change):
  ...
  2. Settable-key allowlist (`_SETTABLE_KEYS`) — anything outside this set
     raises `PermissionError("...not settable...")`. Notably `vault_path` is
     NOT settable from an MCP session (clients must not reroot the vault).
"""

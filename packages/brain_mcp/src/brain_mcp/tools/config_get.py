...
The MCP server doesn't have access to the env Mapping + cli_overrides that
`brain_core.config.loader.load_config()` requires, so we snapshot config the
same way `brain://config/public` does: build a defaults-backed `Config()` and
overlay the session's `vault_root`. That gives the client a consistent view
of the session's resolved settings, without needing a `.brain/config.json`
on disk.

...
The only MCP entry point that actually mutates the vault. Looks up the
...
async def handle(arguments: dict[str, Any], ctx: ToolContext) -> list[types.TextContent]:
    ...
    # Re-express applied_files as vault-relative POSIX strings so the MCP
    # client sees a stable, cross-platform representation.

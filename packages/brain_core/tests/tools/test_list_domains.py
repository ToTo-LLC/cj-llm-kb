...
def _mk_ctx(
    ...
    *,
    ...
    union). Mirrors Plan 13 Task 1's None-policy strictness — every call
    site must declare its config shape. The None-config raise behavior is
    pinned in ``test_errors_raise_if_no_config.py`` (Plan 15 Task 8); this
    fixture intentionally cannot construct a None-config context.
    """
    return ToolContext(
        vault_root=vault,

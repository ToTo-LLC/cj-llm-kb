...
const Topbar = ({ scope, setScope, view, activeThread, mode, setMode, costToday, budget, theme, setTheme, toggleRail, railOpen }) => {
  ...
  return (
    <div className="topbar">
      <div className="brand">
        <svg className="brand-mark" viewBox="0 0 120 120" fill="none" stroke="currentColor" aria-hidden="true">
          <circle cx="36" cy="60" r="20" strokeWidth="6" />
          <circle cx="84" cy="60" r="10" fill="var(--brand-ember)" stroke="none" />
          <line x1="56" y1="60" x2="74" y2="60" strokeWidth="6" strokeLinecap="round" />
        </svg>
        <div className="name">
          <span className="wm">brain<span className="dot">.</span></span>
...
// NOTE (v3): TweaksPanel removed — it was a design-time iteration overlay, not a ship feature.
// Theme + Density now live in Settings → General. Rail-mode ("Pop in" / "Badge") dropped
// per v3 cleanup C1 — one default (pop-in) is enough.

Object.assign(window, { Topbar, LeftNav, Toasts });

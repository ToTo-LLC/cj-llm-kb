...
- Every query/chat thread has an **active scope**: one domain, or `cross-domain` (requires a one-time confirmation warning when the scope crosses into a privacy-railed domain). The modal fires only when the scope contains ≥2 domains AND ≥1 of them is in `Config.privacy_railed`; single-domain railed access is already gated by §7's explicit-inclusion rule. Acknowledgment persists in `Config.cross_domain_warning_acknowledged: bool` (default `False`); a "Show cross-domain warning" toggle in Settings → Domains lets users re-enable the prompt. **The acknowledgment is vault-level, not per-thread** — once a user clicks "I understand" in any thread, the modal stays dismissed across all subsequent threads until the user re-enables it from Settings. Per-thread cross-domain confirmation is an INTENTIONAL ARCHITECTURAL NO; re-litigating this requires a spec amendment, not a "we didn't get to it" deferral. The rationale: per-thread prompts train users to click-through reflexively, defeating the purpose. Vault-level + Settings re-enable preserves user agency without creating prompt fatigue. (Plan 16 D36 — held the line on this under full-production review.)
...
- **Per-domain × per-category autonomy** (Plan 16 T37-T41): `Config.autonomous: dict[str, AutonomyCategoryFlags]` reshapes the day-one flat `AutonomousConfig` into a per-domain map keyed by domain slug. `AutonomyCategoryFlags` carries 5 booleans per domain (`new_files`, `edits`, `index_entries`, `concepts`, `draft`); all default `False` so out-of-the-box every patch stages for approval. The autonomy gate (`should_auto_apply(patchset, config, *, domain)`) implements a **HYBRID** algorithm: the patch's *member fields* (`new_files`, `edits`, `index_entries`) gate via the same-named flags (intersection semantics — any populated member field requires its flag); the patch's *category* (`PatchCategory.CONCEPTS`, `PatchCategory.DRAFT`) requires its category flag in addition. `PatchCategory.OTHER` never auto-applies. Read-time backwards-compat migrates legacy flat `AutonomousConfig` to the per-domain shape (with conservative D-2 dropping `entities=True` rather than carrying it forward — the legacy `entities` flag was an early experiment that didn't survive to production). An explicit `brain config migrate` CLI rolls over on-disk `config.json` files for users who want the in-place migration; users who never run the CLI get the read-time fallback transparently. Per CLAUDE.md principle #3, all flags ship `False` out-of-the-box — the user must opt into auto-apply per-domain × per-category.
- **HYBRID autonomy gate algorithm** (Plan 17 T12 — implementation footnote for the bullet above; canonical code in `brain_core.autonomy.should_auto_apply`):
  1. If `patchset.category == PatchCategory.OTHER`, return `False` (preserved invariant — OTHER is the default and never auto-applies).
  2. Look up `flags = config.autonomous.get(domain)`. Missing entry ⇒ return `False` (all-False semantics; no domain entry means stage everything for that domain).
  3. **Member-field rule.** For each populated `PatchSet` member field, the same-named flag on `AutonomyCategoryFlags` must be `True`: `new_files` ↔ `flags.new_files`, `edits` ↔ `flags.edits`, `index_entries` ↔ `flags.index_entries`. Empty member fields impose no requirement.
  4. **Category rule (CONCEPTS / DRAFT only).** If `patchset.category == PatchCategory.CONCEPTS`, additionally require `flags.concepts == True`. If `patchset.category == PatchCategory.DRAFT`, additionally require `flags.draft == True`. `PatchCategory.INGEST`, `PatchCategory.ENTITIES`, and `PatchCategory.INDEX_REWRITES` impose **no** category-level requirement — their member-field flags govern alone.
  5. **ANY-False rule.** Return `True` iff *every* required flag from steps 3 and 4 is `True`. If any single required flag is `False`, the entire patch stages (no partial auto-apply — a multi-member patch is atomic across the gate).
...
- **Per-domain budget caps + rate limits** (Plan 16 T26-T32): `Config.budget.per_domain: dict[str, BudgetOverride]` lets users set per-domain `daily_usd` ceilings on top of the global cap (a domain's effective ceiling is `min(global, override)`). `PerDomainBudgetGuard.check_for(domain, config)` fires before every LLM call at the production entry points (`brain_core.chat.session`, `brain_core.chat.autotitle`, `brain_core.chat.fork`, `brain_core.ingest.classifier`, `brain_core.ingest.pipeline`, `brain_core.tools.ping_llm`); a domain that's exhausted its budget raises before token spend. Per-domain rate limits live at `Config.providers["anthropic"].rate_limit_per_domain: dict[str, RateLimitOverride]` and are enforced via a leaky-bucket gate inside `AnthropicProvider` (one bucket per `(provider, domain)` pair, registered at module level). Both caps are exposed in Settings → Domains as per-row inputs and via `brain config set` wildcards (`budget.<slug>.daily_usd`, `providers.anthropic.rate_limit_per_domain.<slug>.<field>`). Multi-domain chats currently no-op the per-domain guard (passes `None` domain — the budget guard is a single-domain construct); single-domain chats use `config.domains[0]`. Rate limiter chosen as leaky-bucket over sliding-window: queues smooth bursty traffic, single-decision gate (tokens-per-second + capacity), and Anthropic's own server-side limiter is bucket-shaped (matches the abstraction the upstream provider models).
  - **Multi-domain per-call enforcement policy** (Plan 17 T13 / D4): when `len(Config.domains) > 1`, chat-mode call sites (`chat.session`, `chat.autotitle`, `chat.fork`) pass `domain=None` into `PerDomainBudgetGuard.check_for(...)`. The guard short-circuits on `domain is None` — no per-domain ledger lookup, no enforcement. This is deliberate, not a bug: a multi-domain chat has no canonical per-call domain to attribute spend against, so per-domain caps (which are scoped-attribution constructs) are meaningful only in single-domain mode. The global daily/monthly ceilings still apply on every call, so spend is never unbounded. Re-litigation path: a future spec amendment may introduce multi-domain attribution semantics (e.g., proportional split across active domains, scope-aware per-call accounting, or a "primary" domain field on the chat thread) — but that's a deliberate scope decision deferred today, not a TODO.

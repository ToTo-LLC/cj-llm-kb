...
**Status:** ✅ Complete — tag `plan-22-watched-folders-sync` cut at T17 HEAD on 2026-05-12. Push deferred per D12 (user authorization required at T18).

**Closure summary.** Plan 22 is the first "real new subsystem" plan since Plan 09 (v0.1.0). 17 substantive tasks + 3 mini-fixes (T10.5 lookup-gap, T12 fix-up resync wiring, T14.5 dry_run mode) landed in 40+ commits across 4 themes. **Theme A — Foundation (T0-T1).** Spec §4 + 4 new optional frontmatter fields (`source_path` / `orphaned` / `orphaned_at` / `watched_folder_id`); spec §5 + "Watched folders" subsection documenting lifecycle + D1 overwrite contract + 7-tool surface; spec §10 + safety-rails bullet (backup trigger + `include_orphans` default-filter). `WatchedFolder` Pydantic model + `Config.watched_folders: list[WatchedFolder]` field. **Theme B — Backend (T2-T10).** `IngestPipeline.update_source` (re-ingest preserving slug + frontmatter `domain`) + `IngestPipeline.mark_orphaned` (non-destructive). `scope_guard(path, *, allowed_domains, include_orphans=False)` extended. 7 watched-folder tools registered (`brain_watch_folder` / `brain_unwatch_folder` / `brain_list_watched_folders` / `brain_list_orphans` / `brain_resync_folder` / `brain_restore_orphan` / `brain_delete_orphan`). `WatchedFolderWatcher` core in `brain_core.watch.folder_watcher` mirrors `ConfigWatcher` D7 symmetric pattern; brain_api lifespan + brain_mcp `_cached_ctx` both register watchers with parallel `_build_watched_folder_watcher` + `_restart_watched_folder_watcher` helpers. `pre_watched_folder_sync` joined `_VALID_TRIGGERS` in `backup_create.py`. **T10.5 (CORRECTNESS GATE).** Lookup-gap mini-fix wired `source_path` + `watched_folder_id` kwargs through `IngestPipeline.ingest()`; create-then-modify lifecycle test catches the live-sync watcher path. **Theme C — Frontend (T11-T15).** brain-ui-designer delivered 6 ASCII-wireframe mockups + README at `docs/design/plan-22/`. `panel-watched-folders.tsx` + `panel-orphans.tsx` + `watched-folders-topbar-indicator.tsx` + 3 confirmation modals + Bulk Import → "Watch this folder" CTA in `step-apply.tsx`. T12 fix-up wired the Resync button after verification cleared an implementer false-negative. T14.5 added `dry_run` mode to `brain_watch_folder` for the watch-enable modal cost-preview flow (7 pin tests). **Theme D — Closure (T16-T17).** Playwright e2e `watched-folders.spec.ts` covers enable / disable / orphan / bulk flows with axe gates. `scripts/demo-plan-22.py` 21-gate demo prints `PLAN 22 DEMO OK`. `packages/brain_api/tests/test_tools_listing.py` constant cleared from 38 (Plan 16 baseline) to 45 (Plan 22 +7 tools) + 7 spot-check assertions for new tool names + test function renamed `test_lists_thirty_six_tools_after_issue_17` → `test_lists_all_registered_tools`.

**Verification receipts.**

- `scripts/demo-plan-22.py` — 21/21 gates pass; final stdout `PLAN 22 DEMO OK`.
- `packages/brain_api/tests/test_tools_listing.py` — 3/3 tests pass with the new 45-tool constant.
- All Plan 22-introduced tests in `packages/brain_core/tests/watch/`, `packages/brain_core/tests/config/`, `packages/brain_core/tests/ingest/`, `packages/brain_core/tests/tools/test_{watch,unwatch,list_watched,list_orphans,resync,restore_orphan,delete_orphan}_folder.py`, `packages/brain_api/tests/test_watched_folders_integration.py` pass per their respective T1-T10 outcome receipts.
- Playwright e2e `apps/brain_web/tests/e2e/watched-folders.spec.ts` covers enable / disable / orphan / bulk flows with axe gates per the T16 outcome receipt.

**Bumps and deltas.** Zero spec rollback risk. Zero schema breaks (the 4 new frontmatter fields are all optional with safe defaults; `Config.watched_folders: list[WatchedFolder] = []` defaults to empty so existing configs load unchanged). Zero new external dependencies (`watchdog` already in scope from Plan 16 T35 ConfigWatcher). Tool surface: 38 → 45 (+7 watched-folder tools). Frontend surface: +3 Settings panels (watched folders + orphans + topbar indicator) + 3 confirmation modals + 1 Bulk Import CTA. Backend surface: +1 watcher (`WatchedFolderWatcher`) + 2 pipeline methods (`update_source` + `mark_orphaned`) + 1 scope-guard kwarg (`include_orphans`) + 1 backup trigger (`pre_watched_folder_sync`).

**Backlog forward (Plan 23 candidate scope).** See `tasks/todo.md` tail block. 19 items across 5 sections (priority order): 1 critical backend bug (orphan-walk ValidationError), 9 UX polish items (Theme C concerns), 4 architectural cleanups (Theme B carry-forwards), 1 dev-loop friction (workspace editable flip from Plan 21), 4 preserved NOT-DOING items (unchanged since Plan 17). Most substantial carry-forward block since Plan 16. Plan 23 will be authored just-in-time once the user reviews + locks scope.

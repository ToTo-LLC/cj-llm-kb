...
- **Plan-author meta-lesson (Tasks 14, 8)**: (1) plan text referenced imagined APIs (`PromptLoader` class, `FakeLLMProvider.queue_response`) instead of real Plan 02 signatures (`load_prompt` function, `.queue`). Verify real signatures before writing future tasks. (2) Tasks 8, 14, 16 all triggered false "mypy pre-existing errors" / "no .venv" from implementers running mypy / find from the wrong cwd. Add explicit `cd packages/brain_core` to the per-task self-review checklist in Task 25.
- **Task 17 `_build_effective_registry` double subset.** Two sequential `.subset(allowlist=...)` calls when `open_doc_path is None`. Single-pass filter would read cleaner. Behavior correct.

...
| 11 | [Persistent Config + Per-Domain Overrides + Privacy Rail Generalization](./plans/11-persistent-config.md) | 📝 Ready for execution (drafted 2026-04-27, 16 decisions locked) | Atomic+locked+backup `save_config()` closes the in-memory-only gap (`#27`); five mutation tools persist to disk; loader gains `config.json → .bak → defaults` fallback. `Config.privacy_railed: list[str]` generalizes the rail (still requires `personal`); `Config.domain_overrides: dict[str, DomainOverride]` + `resolve_llm_config()` resolver thread per-domain LLM/autonomy overrides through ingest+chat. Frontend: `panel-domains.tsx` grows per-row override editor + privacy-rail toggle; `app-store.scopeInitialized` + `useDomains().activeDomain` give first-mount scope hydration (closes Plan 10 Task 7 descope). Spec §4 + §7 amended. 10 tasks, 8 demo gates. Closes `docs/v0.1.0-known-issues.md` item #27. | brain-core-engineer, brain-frontend-engineer, brain-test-engineer |
